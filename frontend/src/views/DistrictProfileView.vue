<template>
  ...
  <div v-else-if="!district" class="text-center py-20 bg-white rounded-lg border border-gray-100 shadow-sm max-w-xl mx-auto mt-10 px-4">
    ...
    <button @click="router.push('/districts')" class="mt-6 px-6 py-2.5 text-xs bg-gray-900 text-white font-bold rounded-md hover:bg-black transition-all shadow-sm active:scale-95">
      Return to Directory
    </button>
  </div>
  ...
  <div v-else class="max-w-6xl mx-auto pb-20 px-4 sm:px-6 mt-4 sm:mt-6 space-y-4">
    ...
    <div class="bg-white rounded-lg shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border border-gray-100 p-5 sm:p-6">
      <div class="flex flex-col md:flex-row gap-5 items-start md:items-center">
        ...
        <!-- District Icon -->
        <div class="shrink-0">
          <div class="w-16 h-16 rounded-md shadow-sm overflow-hidden flex items-center justify-center border border-gray-200"
               :style="{ backgroundColor: district.theme_color || '#3b82f6' }">
            <span class="text-2xl font-black text-white uppercase">{{ district.district_name.charAt(0) }}</span>
          </div>
        </div>
        ...
        <div class="flex flex-wrap gap-2 w-full md:w-auto pt-4 md:pt-0 border-t border-gray-100 md:border-0 mt-2 md:mt-0 shrink-0 relative group">
          <button class="w-full md:w-auto px-4 py-2 bg-gray-900 hover:bg-black text-white font-bold text-xs rounded-md transition-all flex items-center justify-center gap-2 shadow-sm active:scale-95">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
            Add Station
          </button>
          
          <!-- Dropdown Choice -->
          <div class="absolute right-0 top-full mt-2 w-48 bg-white border border-gray-100 rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 overflow-hidden">
    ...
    <!-- Main Content Layout (stacked: metrics/leader on top, stations below) -->
    <div class="space-y-4">
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-5 grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div class="flex flex-col items-center p-3 bg-gray-50 rounded-md border border-gray-100">
          ...
          <div class="flex flex-col items-center p-3 bg-emerald-50 rounded-md border border-emerald-100">
          ...
          <div class="flex flex-col items-center p-3 bg-red-50 rounded-md border border-red-100">
          ...
          <div class="flex flex-col items-center p-3 bg-amber-50 rounded-md border border-amber-100">
      ...
      <!-- Assigned Stations (full width) -->
      <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-5 sm:p-6 min-h-[400px]">
        ...
        <div v-if="filteredChurches.length === 0" class="text-center py-12 text-gray-400 border border-dashed border-gray-200 rounded-md">
          <p class="font-bold uppercase tracking-widest text-[10px]">No results found.</p>
        </div>

        <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            <div v-for="church in filteredChurches" :key="church.id" @click="$router.push(`/churches/${church.id}`)" class="flex items-center gap-3 p-3 rounded-md border border-gray-100 hover:border-indigo-200 hover:shadow-sm hover:bg-gray-50/50 transition-all cursor-pointer group">
    ...
    <Teleport to="body">
    <div v-if="isSelectionModalOpen" class="fixed inset-0 z-[9999] flex flex-col items-center justify-center py-20 px-4 md:px-10 bg-gray-900/80 backdrop-blur-sm">
      <div class="bg-white rounded-lg shadow-xl w-full max-w-xl flex flex-col relative my-auto animate-in slide-in-from-bottom duration-300">
        ...
        <div class="p-5">
           <div class="relative mb-4">
              ...
              <input v-model="selectionSearch" type="text" placeholder="Search by name or address..." class="w-full pl-9 pr-4 py-2 bg-white border border-gray-200 rounded-md text-xs font-bold focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all">
           </div>

           <div class="overflow-y-auto max-h-[50vh]">
             <div v-if="filteredAvailable.length === 0" class="py-12 text-center text-gray-400 border border-dashed border-gray-200 rounded-md">
             ...
             <div v-else class="space-y-2">
                <button v-for="c in filteredAvailable" :key="c.id" 
                   ...
                   class="w-full flex items-center justify-between p-3 rounded-md border border-gray-100 hover:border-indigo-200 hover:bg-indigo-50/50 transition-all group text-left disabled:opacity-50">
    ...
    <div v-if="isAddChurchModalOpen" class="fixed inset-0 z-[9999] flex flex-col items-center justify-center py-20 px-4 md:px-10 bg-gray-900/80 backdrop-blur-sm">
      <div class="bg-white rounded-lg shadow-xl w-full max-w-lg flex flex-col relative my-auto animate-in slide-in-from-bottom duration-300">
        <div class="px-5 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 rounded-t-lg">
