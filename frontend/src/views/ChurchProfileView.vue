  ...
  <div v-else class="max-w-6xl mx-auto pb-20 px-4 sm:px-6 mt-4 sm:mt-6 space-y-4">
      ...
      <div class="flex flex-col md:flex-row gap-5 items-start md:items-center">
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex items-center gap-2 text-xs font-bold text-gray-500 uppercase tracking-wider truncate">
            <span v-if="church.district_id" @click="router.push(`/districts/${church.district_id}`)" class="cursor-pointer hover:text-indigo-600 transition-colors">Dist: {{ church.district_name }}</span>
            <span v-else class="text-gray-400">Unassigned District</span>
            <span v-if="church.mother_church_name" class="mx-1">•</span>
            <span v-if="church.mother_church_name" class="truncate">Mother: {{ church.mother_church_name }}</span>
          </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <div class="lg:col-span-1 space-y-4">
        ...
        <div v-if="church.pioneer_pastor_name" class="bg-white rounded-[1.5rem] shadow-sm border border-gray-100 overflow-hidden">
          <div class="px-5 py-3.5 border-b border-gray-50 bg-gray-50/30">
             <h3 class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Pioneer / Founder</h3>
          </div>
          <div class="p-5 flex items-center gap-3">
             <div class="w-10 h-10 rounded-lg overflow-hidden bg-indigo-50 border border-indigo-100 shrink-0 flex items-center justify-center font-black text-indigo-600 uppercase">
               {{ church.pioneer_pastor_name.charAt(0) }}
             </div>
             <div class="min-w-0">
               <h4 class="font-bold text-gray-900 text-xs uppercase truncate">{{ church.pioneer_pastor_name }}</h4>
               <p class="text-[10px] font-bold text-gray-500 uppercase truncate">Established Station</p>
             </div>
          </div>
        </div>
             ...
             <div v-for="assign in assignments" :key="assign.id" class="relative pl-5">
                ...
                <div>
                  ...
                  <div class="flex justify-between items-start mb-1">
                      <h4 class="text-sm font-black text-gray-900 uppercase tracking-tight">Ptr. {{ assign.pastor_name }}</h4>
