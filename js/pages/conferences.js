    ...
    <div class="conf-card">
      <div class="conf-head">
        ...
        <div class="btn-action-group">
          <a href="scanner.html?confId=${c.id}" class="btn btn-primary btn-action" style="color:#fff;">
            <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M3 7V5a2 2 0 0 1 2-2h2"/><path d="M17 3h2a2 2 0 0 1 2 2v2"/><path d="M21 17v2a2 2 0 0 1-2 2h-2"/><path d="M7 21H5a2 2 0 0 1-2-2v-2"/></svg>
            Scan
          </a>
          ${(typeof authService !== 'undefined' && authService.getCurrentUser()?.role === 'Staff') ? '' : `
          <button class="btn btn-ghost btn-action" onclick="openEditConf('${c.id}')">
          ...
          <button class="btn btn-ghost btn-action" style="color:var(--red);" onclick="openDeleteConf('${c.id}','${esc(c.title)}')">
            <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>
            Delete
          </button>
          `}
        </div>
...
function bindEvents() {
  ...
  document.getElementById('btn-add').onclick = () => { if(!isStaff) openCreateConf() }
