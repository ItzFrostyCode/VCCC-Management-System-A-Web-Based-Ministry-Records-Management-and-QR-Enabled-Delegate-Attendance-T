...
class BadgesController {
    ...
    initFilterSelectors() {
        ...
        if (distEl) this.selFilterDist = createSearchSelect(distEl, this.districts.map(d => ({ value: d.id, label: d.district_name })), 'District', (v) => {
            this.filterDistVal = v || null; this.filterChurchVal = null;
            const fc = this.churches.filter(c => !this.filterDistVal || c.district_id === this.filterDistVal);
            if (this.selFilterChurch) {
                this.selFilterChurch.setOptions([{ value: '', label: 'All churches' }, ...fc.map(c => ({ value: c.id, label: c.church_name }))]);
                this.selFilterChurch.reset();
            }
            this.applyFilters();
        });
        if (churchEl) this.selFilterChurch = createSearchSelect(churchEl, [{ value: '', label: 'All churches' }, ...this.churches.map(c => ({ value: c.id, label: c.church_name }))], 'All churches', (v) => {
            this.filterChurchVal = v || null;
            this.applyFilters();
        });
    }
    ...
    renderTools() {
        ...
        if (!b.querySelector('.editor-toolbar')) { 
            ...
            this.selFieldEditor = createSearchSelect(document.getElementById('f-sel-box'), [
            ...
            ], 'Select...', (v) => { if (v) { this.activeFieldKey = v; this.renderTools(); } }); 
            if (this.activeFieldKey) this.selFieldEditor.setValue(this.activeFieldKey);
        }
