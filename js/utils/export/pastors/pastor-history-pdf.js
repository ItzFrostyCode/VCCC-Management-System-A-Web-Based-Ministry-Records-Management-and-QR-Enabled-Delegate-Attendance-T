 ...
 * Uses jsPDF and jsPDF-AutoTable (loaded via CDN in pastor-view.html)
 */

export async function exportPastorHistoryPDF(data) {
    const { pastor, history, ranks, trainings, pioneered } = data;
    
    // Check if jsPDF is available (it should be since we'll add it to the HTML)
    if (typeof window.jspdf === 'undefined') {
        throw new Error('jsPDF library not loaded. Please ensure the CDN is active.');
    }
    ...
    doc.text(`Current Status: ${pastor.current_status_code.toUpperCase()}`, midCol, y);
    doc.text(`Rank: ${ranks && ranks.length > 0 ? ranks[0].rank_code : 'Worker'}`, rightCol, y);

    y += 7;
    doc.text(`Birthdate: ${pastor.birthdate || 'Unknown'}`, leftCol, y);
    ...
    doc.save(`Pastoral_Record_${pastor.full_name.replace(/\s+/g, '_')}.pdf`);
}
